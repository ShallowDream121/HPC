...
template <size_t PPWI> class IMPL_CLS final : public Bude<PPWI> {
  ...
  static void fasten_main(size_t group,                                        //
    ...
    std::for_each(std::execution::par_unseq, 0, PPWI, [&](int l) {
      int ix = group * PPWI + l;
    ...
    std::for_each(std::execution::par_unseq, ligand.begin(), ligand.end(), [&](const Atom &l_atom) {
        ...
        std::for_each(std::execution::par_unseq, protein.begin(), protein.end(), [&](const Atom &p_atom) {
...
public:
  ...
  [[nodiscard]] std::string name() override { return "std17"; };
